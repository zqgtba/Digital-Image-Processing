...
% |.jpg|, |.tif| and |.png|.
tic
demoFolder = fullfile('/home/rocky/Downloads/ncut_mult/ncut_multiscale_1_6','image');
%imsciFolder = fullfile(matlabroot, 'toolbox', 'matlab', 'imagesci');

%%
% Create a |datastore| using the folder paths, and filter which images
% are included in the datastore using the |FileExtensions| Name-Value pair.
ds = datastore({demoFolder}, 'Type', 'image', ...
...
% it does not scale well to a large collection of images. This example uses
% |mapreduce| to accomplish the same task, however, the |mapreduce| method
% is highly scalable to larger collections of images.
...
% * The intermediate values are assoicated with 3 keys, |'Average Hue'|,
